...
GNN (graph neural network) detection is a **draft proof-of-concept**, not production scoring. TRACE remains the sole online detector. This page tracks research status, schema, and the planned Phase 4 inference path.
...
| Live GNN inference service | Not built (Phase 4) |
| Anomalies UI for GNN scores | Not wired |

## Why GNN

TRACE scores per-host temporal surprise. GNN research explores cross-host and cross-process structure: shared binaries, parent/child process trees, and operational dependencies encoded as a fleet graph.
...
- No promoted artifact in `ENABLED_MODELS` and no poll-loop writer for GNN scores; a working research pipeline is not "GNN alerts in prod"
- No `dag_gnn_scores` table or backend handler in prod today
- Graph freshness depends on enrichment edge writer lag
- Evaluation harness is offline only; no dashboard TTD or narratives path for GNN
- TRACE and GNN would coexist as separate finding sources if inference ships later
...
flowchart LR
  ...
  subgraph gnn [GNN research]
    gIn[Graph edges ops procs] --> gModel[GNN encoder]
    gModel --> gOut[Offline scores TBD]
  end
```
