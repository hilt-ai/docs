...
```mermaid
flowchart LR
  enrich[dag-enrichment] --> scores[(dag_anomaly_scores)]
...
| --- | --- | --- | --- |
| `dag_anomaly_scores` | enrichment | ML poll | 20-D features per host/epoch |
| `dag_epoch_tokens` | enrichment | ML + backend | Token arrays and evidence |
| `dag_trace_scores` | ML | backend, frontend | Production findings |
...
ML training reads these offline; no online GNN score writer yet.
