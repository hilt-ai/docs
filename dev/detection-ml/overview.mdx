...
Detection ML writes anomaly scores to ClickHouse. The backend exposes them on the [Anomalies](https://docs.hilt.ai/dashboard/anomalies) page and, through fusion, on LLM [narratives](https://docs.hilt.ai/dashboard/anomalies). This section walks engineers through the full flow. Model math lives in the [dag-detectionML](https://github.com/hilt-ai/dag-detectionML/tree/dev/docs) repo.
...
| --- | --- | --- |
| TRACE online scoring | Prod | Sole entry in `config.ENABLED_MODELS` (`trace`): a hardcoded dict, not an env var |
...
| GNN training pipeline | Research (draft) | AWS Batch, offline only; see [GNN](/detection-ml/gnn) |
| GNN live inference | Not built | Inference service is Phase 4; edge tables exist when enrichment opt-in is enabled |
| TS-VAE + Isolation Forest | Deprecated | Removed from runtime; the repo README still mentions TS-VAE/IF and `dag_ml_scores`, but production scoring uses TRACE and writes `dag_trace_scores` |
...
flowchart LR
  ...
  gnnTables -.future.-> gnnInfer[GNN inference TBD]
```
...
- **Numeric features** (20-D vector) to `dag_anomaly_scores` via [`internal/scoring/`](https://github.com/hilt-ai/dag-enrichment/tree/dev/internal/scoring)
...
The statistical baseline and feature builder in enrichment produce the numeric vector TRACE fuses with token embeddings. ML polls `dag_anomaly_scores` for new epochs; it does not recompute enrichment features.
