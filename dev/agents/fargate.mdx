...
flowchart TB
  subgraph Task["ECS Fargate Task"]
    Installer["dag-fargate-installer sidecar\n(essential=false)\ncopies binary → shared volume, exits"]
    subgraph App["App container"]
      Agent["dag-fargate-agent (PID 1)\ninstalls seccomp filter, PTRACE_SEIZE"]
      Child["customer app\n(fork+exec child, inherits filter)"]
      Agent --> Child
    end
  end
